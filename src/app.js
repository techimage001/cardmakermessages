(() => {
  ...
  async function shareFiles(files, title, fallback) {
    ...
    if (supportsFileShare(files)) {
      try {
        // Send the finished card file and the promotional website link in the
        // same native share action. The destination app decides how it displays
        // the text, but the user does not need to copy the link separately.
        await navigator.share({ files, title, text: message });
        announce('Your card image and website link were sent to the sharing app.');
        return true;
      } catch (error) {
        if (error?.name === 'AbortError') return false;
        // Some browsers advertise file sharing but reject a particular file type
        // or number of files. Use the visible download-and-copy fallback instead.
      }
    }
