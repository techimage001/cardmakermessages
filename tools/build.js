...
function signupModal() {
  return `<div class="modal-backdrop" data-signup-modal hidden>
    <section class="signup-modal" role="dialog" aria-modal="true" aria-labelledby="signupTitle">
      ...
      <p>Create and export three cards first. A free email sign-up then keeps every card, invitation and print size unlocked on this device.</p>
      <form id="signupForm" novalidate>
        ...
        <button class="button button-primary button-large" type="submit">Sign up and continue</button>
        <p class="form-status" data-signup-status role="status" aria-live="polite"></p>
      </form>
      <p class="modal-small">Nothing is charged and no card details are requested. By signing up, you agree to the <a href="/privacy.html">privacy policy</a>.</p>
    </section>
  </div>`;
}
...
<section class="hero editorial-hero">
  ...
  <div class="answer-block answer-block-hero"><p>Choose the occasion, edit every word, tap a complete design and review the finished card before you share, print or download it. No blank canvas and no complicated design controls.</p></div>
...
<section class="section section-dark premium-section"><div class="container"><div class="section-heading light-heading"><p class="eyebrow eyebrow-gold">Easy from the first tap</p><h2>Three clear stages, with your wording editable throughout</h2></div><div class="process-grid"><article><span>1</span><h3>Message</h3><p>Choose the occasion, then edit the final message, recipient, personal detail, signature and cover wording together.</p></article><article><span>2</span><h3>Design</h3><p>Tap a coordinated design while a compact live card stays visible. Change any word without going back.</p></article><article><span>3</span><h3>Review and download</h3><p>Inspect every panel first, then choose a social image, flat printable PDF or folded card PDF.</p></article></div></div></section>
...
function appBody() {
  ...
  const presetButtons = [['floral','Elegant floral'],['minimal','Modern minimal'],['photo','Photo focus'],['bold','Bold celebration'],['luxury','Luxury dark'],['playful','Joyful colour'],['festive','Traditional festive'],['peaceful','Peaceful light'],['botanical','Botanical'],['cute','Gentle illustrated']].map(([v,l])=>`<button type="button" class="preset" data-preset="${v}" aria-pressed="false"><span class="preset-preview preset-${v}" aria-hidden="true"></span>${l}</button>`).join('');
  const editor = `<section id="persistentMessageEditor" class="control-section message-editor-card"><div class="editor-heading"><div><p class="eyebrow">Your wording</p><h2>Edit the card message</h2></div><button id="generateMessages" type="button" class="button button-secondary" aria-expanded="false" aria-controls="messageOptionsPanel">Show message choices</button></div>
    <div class="field-grid message-field-grid"><div class="field field-full"><label for="mainMessage">Your final message</label><textarea id="mainMessage"></textarea></div><div class="field"><label for="recipientSelect">Recipient</label><select id="recipientSelect"></select></div><div class="field"><label for="recipientName">Name, optional</label><input id="recipientName" autocomplete="off" placeholder="For example, Sarah"></div><div class="field field-full"><label for="personalNote">Personal detail, optional</label><input id="personalNote" placeholder="For example, the support you gave me this year"></div><div class="field"><label for="senderName">From or signature</label><input id="senderName" placeholder="For example, James and family"></div><div class="field"><label for="frontHeading">Front heading</label><input id="frontHeading" placeholder="For example, Happy Birthday"></div><div class="field"><label for="coverMessage">Short cover line</label><input id="coverMessage"></div><div class="field"><label for="backMessage">Back-cover note, optional</label><input id="backMessage" placeholder="Created especially for…"></div></div>
  ...
  return `<section class="app-shell"><div class="container"><div class="app-intro"><p class="eyebrow">No design experience needed</p><h1>Create a card without being a designer</h1><p>Choose an occasion, edit every line, tap a complete design and review the finished result before any file opens.</p></div><p id="resumeNotice" class="resume-notice" hidden>Picked up where you left off. Start again whenever you need a blank card.</p>
  <div class="step-tabs" role="navigation" aria-label="Card creation steps"><button class="step-tab" type="button" data-step="1">1. Message</button><button class="step-tab" type="button" data-step="2">2. Design</button><button class="step-tab" type="button" data-step="3">3. Review &amp; download</button></div>
  <div class="app-grid"><aside class="preview-column"><div class="preview-card"><div class="panel-tabs" aria-label="Card panels"><button type="button" data-panel="front">Front</button><button type="button" data-panel="inside-left">Inside left</button><button type="button" data-panel="inside-right">Inside right</button><button type="button" data-panel="back">Back</button></div><div class="canvas-wrap"><canvas id="cardCanvas" role="img" aria-label="Live preview of your personalised card"></canvas></div><p id="previewCaption" class="preview-caption">Front cover. Live preview.</p></div></aside>
  <section class="control-panel"><div data-step-panel="1"><div class="control-section occasion-first"><h2>What are you making?</h2><div class="creation-type-grid"><button type="button" class="creation-type" data-creation-type="card"><span>♡</span><strong>Greeting card</strong><small>Digital, flat or folded</small></button><button type="button" class="creation-type" data-creation-type="invitation"><span>✦</span><strong>Invitation</strong><small>Date, venue and RSVP</small></button><button type="button" class="creation-type" data-creation-type="postcard"><span>▱</span><strong>Postcard</strong><small>Wide photo or message</small></button></div><h3>Choose the occasion</h3><div class="occasion-choices">${cardButtons}${invitationButtons}${postcardButton}</div><div id="customOccasionField" class="field" hidden><label for="customOccasion">Type your event or occasion</label><input id="customOccasion" placeholder="For example, Church anniversary or passing a driving test"></div></div><div data-editor-slot="1"></div><div class="control-section optional-guidance"><h2>Optional message guidance</h2><div class="field field-full tone-field"><span class="field-label">Tone</span><div class="choice-grid">${tones}</div></div>
  <div id="invitationDetails" class="invitation-fields" hidden><div class="invitation-fields-heading"><strong>Event details</strong><span>These appear automatically.</span></div><div class="field-grid"><div class="field field-full"><label for="eventTitle">Event title</label><input id="eventTitle"></div><div class="field"><label for="eventDate">Date</label><input id="eventDate" type="date"></div><div class="field"><label for="eventTime">Time</label><input id="eventTime" type="time"></div><div class="field field-full"><label for="eventVenue">Venue</label><input id="eventVenue"></div><div class="field"><label for="eventRsvp">RSVP</label><input id="eventRsvp"></div><div class="field"><label for="eventHost">Hosted by, optional</label><input id="eventHost"></div></div></div></div><div class="step-footer"><button id="startAgain" class="button button-quiet" type="button">Start again</button><button class="button button-primary" type="button" data-next-step>Continue to design</button></div></div>
  <div data-step-panel="2" hidden><div data-editor-slot="2"></div><div class="control-section"><div class="editor-heading"><div><p class="eyebrow">One tap changes the complete look</p><h2>Pick a design</h2></div><button id="surpriseDesign" type="button" class="button button-secondary">Surprise me</button></div><div class="preset-grid">${presetButtons}</div></div><div class="control-section"><h2>Photo and finishing touches</h2><div class="photo-row"><input id="photoInput" type="file" accept="image/*"><button id="removePhoto" type="button" class="button button-small">Remove photo</button></div><div class="field-grid"><div class="field"><label for="backgroundPicker">Exterior colour</label><input class="colour-input" id="backgroundPicker" type="color"></div><div class="field"><label for="textColourPicker">Exterior text colour</label><input class="colour-input" id="textColourPicker" type="color"></div></div><h3>Inside paper</h3><div class="choice-grid"><button type="button" class="choice" data-inside-paper="white">Plain white</button><button type="button" class="choice" data-inside-paper="ivory">Warm ivory</button></div><p class="help">The front and back keep the selected colour. Inside panels stay white or ivory for readability and easier home printing.</p><details class="more-options"><summary>More design options</summary><h3>Font style</h3><div class="choice-grid"><button type="button" class="choice" data-font="serif">Elegant serif</button><button type="button" class="choice" data-font="sans">Clean modern</button><button type="button" class="choice" data-font="handwritten">Soft handwritten</button></div><h3>Frame</h3><div class="choice-grid"><button type="button" class="choice" data-frame="none">None</button><button type="button" class="choice" data-frame="classic">Classic</button><button type="button" class="choice" data-frame="double">Double</button><button type="button" class="choice" data-frame="arch">Soft arch</button><button type="button" class="choice" data-frame="soft">Subtle</button></div><h3>Inside-left panel</h3><div class="choice-grid"><button type="button" class="choice" data-inside-left="blank">Leave blank</button><button type="button" class="choice" data-inside-left="photo">Use photo</button><button type="button" class="choice" data-inside-left="quote">Add note</button><button type="button" class="choice" data-inside-left="illustration">Original botanical motif</button></div><div class="field"><label for="insideLeftText">Inside-left note</label><textarea id="insideLeftText"></textarea></div><label class="toggle-row"><input id="showWebsite" type="checkbox">Show the website address on the back</label></details></div><div class="step-footer"><button class="button" type="button" data-prev-step>Back</button><button class="button button-primary" type="button" data-next-step>Continue to review</button></div></div>
  <div data-step-panel="3" hidden><div data-editor-slot="3"></div><div class="control-section review-first"><p class="eyebrow">Review before saving</p><h2>See the complete card first</h2><p>Check the front, white or ivory inside panels and back. Download choices appear after you review the finished card.</p><button id="reviewCard" class="button button-primary button-large primary-download" type="button">Review finished card</button></div><div id="downloadWorkspace" hidden><div class="control-section"><h2>How will you use it?</h2><div class="choice-grid output-mode-grid"><button type="button" class="choice" data-output-mode="digital">Digital and social image</button><button type="button" class="choice" data-output-mode="single-print">One-page printable PDF</button><button type="button" class="choice" data-output-mode="folded">Folded card PDF</button></div></div>
  <div id="digitalOptions" class="control-section"><h2>Digital and social size</h2><div class="choice-grid size-choice-grid"><button type="button" class="choice" data-size="square">Instagram square · 1080 × 1080</button><button type="button" class="choice" data-size="portrait">WhatsApp portrait · 1080 × 1350</button><button type="button" class="choice" data-size="landscape">Landscape · 1600 × 900</button><button type="button" class="choice" data-size="story">Instagram Story · 1080 × 1920</button><button type="button" class="choice" data-size="facebook">Facebook or LinkedIn · 1200 × 630</button><button type="button" class="choice" data-size="pinterest">Pinterest pin · 1000 × 1500</button></div><div class="download-card"><strong id="downloadSummary"></strong><span>The finished image opens in a new tab for viewing.</span></div><button id="downloadPng" type="button" class="button button-primary button-large primary-download">Open high-resolution PNG</button><div class="secondary-actions"><button id="downloadJpg" class="button" type="button">Open JPG</button><button id="shareImage" class="button" type="button">Share card</button><button id="copyImage" class="button" type="button">Copy image</button><button id="copyMessage" class="button" type="button">Copy message</button><button id="shareLink" class="button" type="button">Share link</button><button id="whatsappShare" class="button" type="button">WhatsApp text</button></div></div>
  <div id="singlePrintOptions" class="control-section" hidden><h2>One-page printable PDF size</h2><div class="choice-grid size-choice-grid"><button type="button" class="choice" data-single-print-size="A4">A4</button><button type="button" class="choice" data-single-print-size="A5">A5</button><button type="button" class="choice" data-single-print-size="A6">A6</button><button type="button" class="choice" data-single-print-size="5x7">5 × 7 inch</button><button type="button" class="choice" data-single-print-size="Letter">US Letter</button></div><button id="downloadSinglePdf" type="button" class="button button-primary button-large primary-download">Open one-page printable PDF</button><p class="info-note">This prints the front design as a single flat page. Use the folded option for a traditional card with inside panels.</p></div>
  <div id="printOptions" class="control-section" hidden><h2>Folded card PDF</h2><h3>Paper</h3><div class="choice-grid"><button type="button" class="choice" data-paper="A4">A4 folded to A5</button><button type="button" class="choice" data-paper="A5">A5 folded to A6</button><button type="button" class="choice" data-paper="Letter">US Letter folded card</button></div><h3>Printing</h3><div class="choice-grid"><button type="button" class="choice" data-print-quality="home">Home printer</button><button type="button" class="choice" data-print-quality="shop">Print shop</button></div><label class="toggle-row"><input id="showFoldMarks" type="checkbox">Show small fold guides for home printing</label><button id="downloadPdf" type="button" class="button button-primary button-large primary-download">Open folded card PDF</button><p class="info-note">The inside defaults to warm ivory for readability and lower ink use. The exterior keeps the selected design.</p></div></div><div class="step-footer"><button class="button" type="button" data-prev-step>Back to design</button><button id="startAgainBottom" class="button button-quiet" type="button">Start another card</button></div></div>
  ...
  <div class="modal-backdrop" id="reviewModal" hidden><section class="review-modal" role="dialog" aria-modal="true" aria-labelledby="reviewTitle"><button class="modal-close" id="closeReview" type="button" aria-label="Close card review">×</button><p class="eyebrow">Complete card preview</p><h2 id="reviewTitle">Review every panel</h2><div class="review-grid"><figure><canvas data-review-panel="front"></canvas><figcaption>Front</figcaption></figure><figure><canvas data-review-panel="inside-left"></canvas><figcaption>Inside left</figcaption></figure><figure><canvas data-review-panel="inside-right"></canvas><figcaption>Inside right</figcaption></figure><figure><canvas data-review-panel="back"></canvas><figcaption>Back</figcaption></figure></div><div class="button-row"><button class="button" id="editFromReview" type="button">Edit card</button><button class="button button-primary" id="continueFromReview" type="button">Continue to download</button></div></section></div>`;
}
