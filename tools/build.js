...
function appBody() {
  ...
  const occasionCategories = {
    birthday:'celebrate', anniversary:'celebrate', congratulations:'celebrate', graduation:'celebrate', retirement:'celebrate', 'good-luck':'celebrate',
    wedding:'love', valentines:'love', engagement:'love', friendship:'love', divorce:'love', apology:'love',
    'new-baby':'beginnings', 'new-home':'beginnings', 'new-job':'beginnings', 'job-promotion':'beginnings',
    'get-well':'support', sympathy:'support', 'thinking-of-you':'support', 'pet-loss':'support',
    'mothers-day':'family', 'fathers-day':'family', 'child-naming':'family', 'teacher-appreciation':'work', thanks:'work', farewell:'work',
    christmas:'seasonal', easter:'seasonal'
  };
  const cardButtons = pagesData.map(page => `<button type="button" class="choice occasion-choice" data-kind="card" data-category="${occasionCategories[page.occasion]||'other'}" data-occasion="${page.occasion}" aria-pressed="false"><span aria-hidden="true">${icons[page.occasion]||'♡'}</span><span>${escapeHtml(page.keyword.replace(/ card messages/i,''))}</span></button>`).join('') + `<button type="button" class="choice occasion-choice custom-occasion-choice" data-kind="card" data-category="other" data-occasion="custom" data-popular="true" aria-pressed="false"><span aria-hidden="true">+</span><span>Custom occasion</span></button>`;
  ...
  <div id="cardMakerWorkspace" class="app-grid"><aside class="preview-column"><div class="preview-card"><div id="stageGuardPrompt" class="stage-guard-prompt sticky-stage-guard" hidden role="alert" aria-live="assertive"><button class="stage-guard-close" type="button" data-guard-close aria-label="Dismiss reminder">×</button><strong id="stageGuardTitle">Complete the previous step</strong><p id="stageGuardMessage"></p><button id="stageGuardAction" type="button" class="button button-primary">Go back</button></div><div class="step-tabs five-step-tabs preview-step-tabs" role="navigation" aria-label="Card creation steps"><button class="step-tab" type="button" data-step="1"><span>1</span>Message</button><button class="step-tab" type="button" data-step="2"><span>2</span>Size</button><button class="step-tab" type="button" data-step="3"><span>3</span>Design</button><button class="step-tab" type="button" data-step="4"><span>4</span>Review</button><button class="step-tab" type="button" data-step="5"><span>5</span>Download</button></div><div class="panel-tabs" aria-label="Card panels"><button type="button" data-panel="front">Front</button><button type="button" data-panel="inside-left">Inside left</button><button type="button" data-panel="inside-right">Inside right</button><button type="button" data-panel="back">Back</button></div><div class="canvas-wrap"><canvas id="cardCanvas" role="img" aria-label="Live preview of your personalised card"></canvas></div><div class="preview-edit-strip"><p id="previewCaption" class="preview-caption">Live preview · edit anything and it updates</p><div class="preview-edit-actions"><button type="button" class="button button-small" data-jump-step="1">Edit words</button><button type="button" class="button button-small" data-jump-step="2">Change size</button><button type="button" class="button button-small" data-jump-step="3">Change design</button></div></div></div></aside>
  <section class="control-panel"><div data-step-panel="1"><div class="control-section occasion-first"><h2>What are you making?</h2><div class="creation-type-grid"><button type="button" class="creation-type" data-creation-type="card"><span>♡</span><strong>Greeting card</strong><small>Digital, flat or folded</small></button><button type="button" class="creation-type" data-creation-type="invitation"><span>✦</span><strong>Invitation</strong><small>Date, venue and RSVP</small></button><button type="button" class="creation-type" data-creation-type="postcard"><span>▱</span><strong>Postcard</strong><small>Wide photo or message</small></button></div><h3>Choose the occasion</h3><section class="occasion-browser" aria-labelledby="occasionBrowserTitle"><div class="occasion-browser-heading"><div><span class="occasion-browser-kicker">Browse card categories</span><strong id="occasionBrowserTitle">Find the right card quickly</strong></div><span id="occasionResultCount" class="occasion-result-count" aria-live="polite"></span></div><div class="occasion-category-chips" role="group" aria-label="Card occasion categories"><button type="button" class="occasion-category-chip is-active" data-occasion-category="popular">Popular</button><button type="button" class="occasion-category-chip" data-occasion-category="celebrate">Celebrate</button><button type="button" class="occasion-category-chip" data-occasion-category="love">Love & relationships</button><button type="button" class="occasion-category-chip" data-occasion-category="beginnings">New beginnings</button><button type="button" class="occasion-category-chip" data-occasion-category="support">Support & care</button><button type="button" class="occasion-category-chip" data-occasion-category="family">Family & faith</button><button type="button" class="occasion-category-chip" data-occasion-category="seasonal">Seasonal</button><button type="button" class="occasion-category-chip" data-occasion-category="work">Work & thanks</button><button type="button" class="occasion-category-chip" data-occasion-category="all">All cards</button></div><div class="occasion-filter-bar"><label>Category<select id="occasionCategory"><option value="popular">Popular occasions</option><option value="celebrate">Celebrate</option><option value="love">Love & relationships</option><option value="beginnings">New beginnings</option><option value="support">Support & care</option><option value="family">Family & faith</option><option value="seasonal">Seasonal</option><option value="work">Work & thanks</option><option value="all">All occasions</option></select></label><label>Search<input id="occasionSearch" type="search" placeholder="Search birthday, sympathy, new home..."></label></div></section><div class="occasion-choices">${cardButtons}${invitationButtons}${postcardButton}</div><div id="customOccasionField" class="field" hidden><label for="customOccasion">Type your event or occasion</label><input id="customOccasion" placeholder="For example, Church anniversary or passing a driving test"></div></div><div data-editor-slot="1"></div><div class="control-section optional-guidance"><h2>Optional message guidance</h2><div class="field field-full tone-field"><span class="field-label">Tone</span><div class="choice-grid">${tones}</div></div>
...
function occasionsBody() {
  const groups = [
    ['Celebrate',['birthday','anniversary','congratulations','graduation','retirement','good-luck']],
    ['Love and relationships',['wedding','valentines','engagement','friendship','divorce','apology']],
    ['New beginnings',['new-baby','new-home','new-job','job-promotion']],
    ['Support and care',['get-well','sympathy','thinking-of-you','pet-loss']],
    ['Family, faith and appreciation',['mothers-day','fathers-day','child-naming','teacher-appreciation','thanks','farewell']],
    ['Seasonal',['christmas','easter']]
  ];
  const sections = groups.map(([title,items]) => { const cards=pagesData.filter(page=>items.includes(page.occasion)).map(occasionCard).join(''); return cards ? `<section class="occasion-category-section"><h2>${title}</h2><div class="grid grid-4">${cards}</div></section>` : ''; }).join('');
  const grouped = new Set(groups.flatMap(group=>group[1]));
  const other = pagesData.filter(page=>!grouped.has(page.occasion)).map(occasionCard).join('');
  return `<section class="page-hero"><div class="container">${breadcrumb([{name:'Home',url:'/'},{name:'All occasions',url:'/occasions.html'}])}<p class="eyebrow">Message library</p><h1>Card Messages for Every Occasion</h1><div class="answer-block"><p>Browse clearly organised card occasions, open an indexable message guide and personalise the wording in the free card maker. Every important occasion page is linked in normal HTML so search engines and visitors can reach it directly.</p></div><a class="button button-primary button-large" href="/app.html">Open the card maker</a></div></section><section class="section section-soft"><div class="container occasion-library">${sections}${other ? `<section class="occasion-category-section"><h2>More occasions</h2><div class="grid grid-4">${other}</div></section>`:''}</div></section>`;
}
