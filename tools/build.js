...
function appBody() {
  ...
  <div data-step-panel="5" hidden>
    ...
    <div id="downloadWorkspace" class="download-workspace" hidden><div class="download-card"><span>Reviewed format</span><strong id="downloadSummary"></strong><small>Change any size above and review again before exporting.</small></div>
      <div id="digitalActions" class="control-section download-actions"><h2>Download or share the digital card</h2><button id="downloadPng" type="button" class="button button-primary button-large primary-download">Download high-resolution PNG</button><div class="secondary-actions"><button id="downloadJpg" class="button" type="button">Download JPG</button><button id="shareImage" class="button" type="button">Share card image</button><button id="copyImage" class="button" type="button">Copy image</button><button id="copyMessage" class="button" type="button">Copy message</button></div><h3>Share the card-maker link</h3><p class="help">The social buttons below always use the permanent Card Maker Messages app link. Use Share card image to attach the picture, then paste the copied permanent link if the social app removes the caption.</p><div class="social-share-grid"><button id="whatsappShare" class="button" type="button">WhatsApp</button><button id="facebookShare" class="button" type="button">Facebook</button><button id="linkedinShare" class="button" type="button">LinkedIn</button><button id="xShare" class="button" type="button">X</button><button id="pinterestShare" class="button" type="button">Pinterest</button><button id="emailShare" class="button" type="button">Email</button><button id="shareLink" class="button" type="button">Share or copy link</button></div></div>
      <div id="singlePrintActions" class="control-section download-actions" hidden><h2>Download the printable card</h2><button id="downloadSinglePdf" type="button" class="button button-primary button-large primary-download">Download one-page printable PDF</button><p class="info-note">The PDF downloads to your device, ready to save or print.</p></div>
      <div id="foldedActions" class="control-section download-actions" hidden><h2>Download the folded card</h2><button id="downloadPdf" type="button" class="button button-primary button-large primary-download">Download folded card PDF</button><p class="info-note">The outside and inside sheets download as a two-page PDF. The inside remains white or warm ivory by default.</p></div>
    </div>
  ...
  <aside id="floatingPreviewDock" class="floating-preview-dock" hidden aria-label="Floating card preview"><canvas id="floatingCardCanvas" role="img" aria-label="Small live preview of your card"></canvas><p>Live preview · edit anything and it updates</p><div class="floating-edit-actions"><button type="button" class="button button-small" data-jump-step="1">Edit words</button><button type="button" class="button button-small" data-jump-step="2">Size</button><button type="button" class="button button-small" data-jump-step="3">Design</button></div><button id="returnToEditor" type="button" class="button button-primary button-small">Back to card editor</button></aside>
