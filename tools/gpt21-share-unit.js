...
promise.then(() => {
  ...
  context.navigator.canShare = undefined;
  const fallbackPromise = context.sharePreparedFiles(files, 'Birthday card', async () => { fallbackCount++; });
  return Promise.resolve(fallbackPromise).then(() => {
    assert.strictEqual(fallbackCount, 1, 'fallback must download once');
    assert(announcements.some(x => x === 'The card was downloaded. Share it from your device.'));
    console.log('GPT21 native share unit QA passed.');
  });
